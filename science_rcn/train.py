    ...
    def _load_data(image_dir, num_per_class, get_filenames=False):
        ...
        for category in sorted(os.listdir(image_dir)):
            ...
            for fname in samples:
                ...
                imarr = imread(filepath)
              
                if len(imarr.shape)==3:
                    imarr = np.mean(imarr, -1)
                
                if imarr.shape[0] != 200 and imarr.shape[1] != 200:
                    # Resize and pad the images to (200, 200)
                    image_arr = imresize(imread(filepath), (112, 112))
                    imarr = np.pad(image_arr,
                                 pad_width=tuple([(p, p) for p in (44, 44)]),
                                 mode='constant', constant_values=0)
                        
                loaded_data.append((imarr, category))
        return loaded_data
...
def get_train_data(data_dir, train_size, test_size,
                         full_test_set=False, seed=5):
...
if __name__ == '__main__':
    logging.basicConfig()
#     
    run_experiment(data_dir='../data/recaptcha/recpatcha/',
                   train_size=20,
                   test_size=20,
                   full_test_set=False,
                   pool_shape=(25, 25),
